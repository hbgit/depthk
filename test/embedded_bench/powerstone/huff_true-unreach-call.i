...
void hinsert(int p,int freq)
{
 ...
 return;
  ...
  if(hroot->next == ((void *)0))
  {
 if(hroot->hfreq > new1->hfreq)
 {
  new1->next = hroot;
  hroot =new1;
  traversal->next =((void *)0);
  return;
 }
 else
 {
...
int main(){
  struct codetype cd,code[256];
  struct nodetype node[512];
