...
if test ${parallel} = 1; then
	...
	depthk_options="--debug -i pagai --only-counter-example --force-check-base-case --solver z3 --memlimit 15g --extra-option-esbmc=\"--error-label ERROR --no-slice --no-bounds-check --constrain-all-states\""
