...
if test ${parallel} = 1; then
  if test ${do_memsafety} = 0; then
    ...
    if test ${do_term} = 1; then
	    ...
	    depthk_options=" --force-check-base-case --solver boolector --memlimit 14g --overflow-check --prp "$property_list"  --extra-option-esbmc=\"--no-div-by-zero-check --force-malloc-success --state-hashing --no-align-check --no-pointer-check --no-bounds-check --overflow-check --no-assertions \""
    elif test ${do_memsafety} = 0; then
        depthk_options="--force-check-base-case --solver boolector --prp "$property_list" --extra-option-esbmc=\"--no-div-by-zero-check --force-malloc-success --state-hashing --no-align-check --floatbv --32 --memory-leak-check --no-assertions \""
    else
        depthk_options="--force-check-base-case --solver z3 --memlimit 14g --prp "$property_list" --memory-safety-category --extra-option-esbmc=\"--no-div-by-zero-check --force-malloc-success --state-hashing --no-align-check --memory-leak-check --no-assertions \""
    fi
fi
