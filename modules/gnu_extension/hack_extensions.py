...
def make_pycparser_compatible(data):
    ...
    while counti < len(listdata):
        ...
        # function - int _IO_vfscanf(_IO_FILE *, const char *, __gnuc_va_list, int *);
        matchfunc_iovfscanf = re.search(r"int _IO_vfscanf\(_IO_FILE \*, const char \*(, __gnuc_va_list), int \*\)",
                                        listdata[counti])
        if matchfunc_iovfscanf:
            listdata[counti] = listdata[counti].replace(matchfunc_iovfscanf.group(1), "")

        # function - int _IO_vfprintf(_IO_FILE *, const char *, __gnuc_va_list);
        matchfunc_iovfprintf = re.search(r"int _IO_vfprintf\(_IO_FILE \*, const char \*(, __gnuc_va_list)\);",
                                         listdata[counti])
        if matchfunc_iovfprintf:
            listdata[counti] = listdata[counti].replace(matchfunc_iovfprintf.group(1), "")

        # Removing ", __gnuc_va_list __arg" from other functions
        matchgnucvalist = re.search(r"(, __gnuc_va_list __arg)", listdata[counti])
        if matchgnucvalist:
            listdata[counti] = listdata[counti].replace(matchgnucvalist.group(1), "")
        ...
        #
        if '__attribute__((noinline))' in listdata[counti].split():  listdata[counti] = listdata[counti].replace(
            '__attribute__((noinline))', '')
        if '__attribute__((noinline));' in listdata[counti].split():  listdata[counti] = listdata[counti].replace(
            '__attribute__((noinline));', ';')

        #
        if '__attribute__((packed))' in listdata[counti].split(): listdata[counti] = listdata[counti].replace(
        ...
        if '((__transparent_union__))' in listdata[counti].split(): listdata[counti] = listdata[counti].replace(
        ...
        #
        if '((__warn_unused_result__))' in listdata[counti].split(): listdata[counti] = listdata[counti].replace(
        ...
        if '((__nothrow__))' in listdata[counti].split(): listdata[counti] = listdata[counti].replace('((__nothrow__))',
                                                                                                      '')  # inttypes.h
        if '((__nothrow__));' in listdata[counti].split(): listdata[counti] = listdata[counti].replace(
            '((__nothrow__));', ';')
        ...
        if listdata[counti].startswith('extern') and '((visibility("default")))' in listdata[
            counti].split():  # SDL_cdrom.h
