...
if grep -i -q "Timed out" ${TMPFILE}; then
  ...
  if test ${IS_MEMSAFETY_BENCHMARK} = 1; then
	...
	#Copy graphml file to grapml folder
	fullfile="$benchmark"
	filename=$(basename "$fullfile")
	filename="${filename%.*}".graphml
	echo "Copying graphml file: " ${TMPGRAPHML} ${GRAPHML_FOLDER}${filename}
	cp ${TMPGRAPHML} ${GRAPHML_FOLDER}${filename}
	

        cd UAutomizer/
	uautomizer_command="python3 ./Ultimate.py "
        uatomizer_prop="${property_list}"
	uautomizer_param=" 32bit precise "
        uautomizer_file=" ${benchmark}"
        uautomizer_graph=" ../${TMPGRAPHML}"
        echo "Checking witness with Ultimate Automizer "
        echo "Command: ""${uautomizer_command} ${uatomizer_prop} ${uautomizer_param} ${uautomizer_file} ${uautomizer_graph} > ${TMPCPALOG} 2>${TMPEXECLOG}"

        ${uautomizer_command} ${uatomizer_prop} ${uautomizer_param} ${uautomizer_file} ${uautomizer_graph} > ${TMPCPALOG} 2>${TMPEXECLOG} &
        cd ../
        ...
        do          
          ...
          if grep -q -E "TRUE" ${TMPCPALOG}; then
            echo "UNKNOWN"
            exit 1;
          elif grep -q -E "FALSE" ${TMPCPALOG}; then
	    exit 1;
            break
          elif grep -q -E "UNKNOWN" ${TMPCPALOG}; then
	    echo "UNKNOWN"	
            exit 1;
